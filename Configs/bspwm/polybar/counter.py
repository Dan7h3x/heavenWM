...
if DEBUG:
    ...
    fh = logging.FileHandler(filename="./info-softwarecounter.log", encoding="utf-8")
...
def get_running_guis():
    wlist = [
        line.split()
        for line in subprocess.check_output(["wmctrl", "-lp"])
        .decode("utf-8")
        .splitlines()
    ]
...
def get_running_proc(process_name_list):
    ...
    for i, p in enumerate(process_name_list):
        try:
            count = int(
                subprocess.check_output(["pgrep", "-c", "-x", p]).decode("utf-8")
            )
...
def PROCESS(process_output=""):
    ...
    combine_counts = get_running_proc(list(sum(combine_proccesses.values(), ())))
